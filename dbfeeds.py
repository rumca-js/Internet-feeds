...
def process_opmls(db_name, all_feeds):
...
def process_all_opml_files(args, directory):
    all_files = find_opml_files(directory)
    print("Found {} files".format(len(all_files)))
    all_feeds = get_feeds(all_files)
    print("Found {} feeds".format(len(all_feeds)))
    process_opmls(args.output_db, all_feeds)
